...
def copy_images(imagePaths, folder, count):
    ...
    for path in imagePaths:
        ...
        label = path.split(os.path.sep)[1].split("_")[1]
        ...
        # image to it
        if (options[labelFolder] < count*config.ECON_CAP):
...
copy_images(trainPaths, config.TRAIN_ECON, 0.8)
copy_images(valPaths, config.VAL_ECON, 0.1)
copy_images(testPaths, config.TEST_ECON, 0.1)