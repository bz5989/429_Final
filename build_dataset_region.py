...
def copy_images(imagePaths, folder, count):
    ...
    for path in imagePaths:
        ...
        # image to it
        if (options[labelFolder] < count*config.REGION_CAP):
...
copy_images(trainPaths, config.TRAIN_REGION, 0.8)
copy_images(valPaths, config.VAL_REGION, 0.1)
copy_images(testPaths, config.TEST_REGION, 0.1)