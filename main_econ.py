...
def main():
    ...
    # create data loaders
    (trainDS, trainLoader) = create_dataloaders.get_dataloader(config.TRAIN_ECON,
        transforms=trainTansform, batchSize=config.FINETUNE_BATCH_SIZE)
    (valDS, valLoader) = create_dataloaders.get_dataloader(config.VAL_ECON,
    ...
    opt = torch.optim.Adam(model.parameters(), lr=config.LR, weight_decay=config.REG2)
    ...
    plt.savefig(config.FINETUNE_PLOT2)
    # serialize the model to disk
    torch.save(model, config.FINETUNE_MODEL2)
