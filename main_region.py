...
def main():
    ...
    # create data loaders
    (trainDS, trainLoader) = create_dataloaders.get_dataloader(config.TRAIN_REGION,
        transforms=trainTansform, batchSize=config.FINETUNE_BATCH_SIZE)
    (valDS, valLoader) = create_dataloaders.get_dataloader(config.VAL_REGION,
    ...
    headModel = nn.Sequential(
        ...
        nn.ReLU(),
        nn.Dropout(0.5),
        nn.Linear(128, len(trainDS.classes))
    )
    model.fc = headModel
    ...
    opt = torch.optim.Adam(model.parameters(), lr=config.LR, weight_decay=config.REG2_REGION)
    ...
    plt.savefig(config.FINETUNE_PLOT3)
    # serialize the model to disk
    torch.save(model, config.FINETUNE_MODEL3)
